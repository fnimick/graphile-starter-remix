<div class="grid grid-cols-1 gap-x-2 md:grid-cols-3">
  <div class="col-span-2 space-y-2">
    <h1 class="text-3xl" data-cy="homepage-header">
    ...
    <div>
      If the page hangs (or shows a connection error) this is likely because the
      Svelte server has encountered an issue. Please check your Vite terminal
      and reload the page.
    </div>

    <h4 class="text-xl">Emails</h4>
    <div>
      <strong>
      ...
      where you can browse them. Keep an eye on the terminal; when an email is triggered
    ...
    <h4 class="text-xl">Initial migration</h4>
    <div>
      We use <code>graphile-migrate</code> in this project to manage database
      migrations; this allows you to change the database very rapidly by just
      editing the current migration file:{" "}
      <code>migrations/current.sql</code>. This file should be written in an
      idempotent manner so that it can be ran repeatedly without causing issues.
    </div>
    <div>
      We've committed the first migration for you (which builds the user
      system), but should you wish to customize this user system the easiest way
      is to run <code>yarn db uncommit</code> which will undo this initial
    ...
    <div>
      <p>
        <b>WARNING</b>: <code>graphile-migrate</code> is experimental, you may want
        to find a different migration solution before putting it into production
        (your sponsorship will help make this project stable faster).
      </p>
    </div>

    <h4 class="text-xl">isAdmin</h4>
    <div>
      The <code>isAdmin</code> flag doesn't do anything in this starter, but you
      can use it in your own applications should you need it.
    </div>

    <h4 class="text-xl">What now?</h4>
    <div>
      To get started, click "Sign in" at the top right, then choose "Create One"
      to create a new account.
    </div>
    <div>
      When you're happy, you can add database changes to{" "}
      <code>current.sql</code> and see them reflected in the GraphiQL interface
      a{" "}
    ...
    <h3 class="text-2xl">Further notes</h3>

    <div>
      <span class="italic">You can read this later</span>. The important things
      were above; below is additional information worth a read when you're done
      experimenting.
    </div>

    <h4 class="text-xl">Making it yours</h4>
    <div>
      This project isn't intended to be <code>git clone</code>'d; instead you
      should start a new git repository with the code from the latest release:
    </div>
    ...
    <h4 class="text-xl">Production readiness</h4>
    <div>
      ...
      operations" (a.k.a. "persisted queries").
    </div>
    ...
    <div>
      ...
      this dot disappears. Realtime ✨🌈
    </div>

    <h4 class="text-xl">Server-side rendering (SSR)</h4>
    <div>
      If you disable JS and reload the page you should see the content is still
      displayed (this "server-side rendering" is important to ensuring that your
      users have the best low-latency experience of your website, and that the
      majority of search engines can index its content).
    </div>

    <h4 class="text-xl">Duplicate emails</h4>
    <div>
      To prevent people blocking legitimate users from registering, we only
      prevent the same email address being used multiple times when it has been
      verified by someone. It's up to your application to turn features off/etc
      when the user does not have any verified email addresses (should you
      desire this).
    </div>

    <h4 class="text-xl">Email in production</h4>
    <div>
      We've configured the project to use Amazon SES to send emails in
      production (you will need to create the relevant resources and provide the
      relevant secrets to use this); however it can easily be reconfigured to
      use your email transport service of choice.
    </div>
    ...
    <h4 class="text-xl">One-time clone: no such thing as breaking changes</h4>
    <div>
      It's expected that you'll do a one-time clone of this project to base your
      project off, and that you will not keep your project up to date with this
      one. As such, we can make any changes we like to this project without
      breaking your project.
    </div>
  </div>
  <div class="space-y-2">
    <h4 class="text-xl">PostGraphile relies on your support</h4>
    <div>
      <b>A message from Benjie</b>
    </div>
    <div>
      I really hope that this project wows you 😍 and saves you huge amounts of
      time. I've certainly poured a lot of time into it!
    </div>
    <div>
